...
class Game {
     ...
     * @returns {object}
     */
    calculateMoveLeft(pawnPosition) {
     ...
     * @returns {object}
     */
    calculateMoveRight(pawnPosition) {
     ...
     * @returns {object}
     */
    calculateMoveDown(pawnPosition) {
     ...
     * @returns {array}
     */
    calculateMoveAllDirections(pawnPosition) {
    ...
    generateAvailableSquares(pawnPosition) {
        const squares = this.calculateMoveAllDirections(pawnPosition);
    ...
    pathFinder(pawnPosition) {
        let visitedSquares = new Set(); // THIS PROBABLY BREAKS THE FINDER
     ...
     */
    hopsOffBoard(row, col, direction) {
     ...
     */
    canJumpOverPawn(currentPos, destPos) {
        let correctedDestination = [];
        let direction = this.directionOfTravel(currentPos, destPos);
     ...
     */
    movePawn(destination, finding = false) {
        let moveResults = this.calculateMovePawn(destination);
        const currentPosition = this.turn === 'white' ? this.whitePos : this.blackPos;
        const jumpOverPawn = (destination) => {
            let jump = this.canJumpOverPawn(currentPosition, destination);
            if (jump.success && jump.canJump) {
                if (this.turn === "white") {
                    ...
                    this.board[jump.destination[0]][jump.destination[1]].occupiedBy = this.turn;
    ...
    calculateMovePawn(destination) {
        ...
        let isBlocked = noWallPresent(currentPos, this.directionOfTravel(currentPos, destination));
    ...
    checkForImpedingWalls(targetLocation, orientation) {
        /*
            even row odd col = v
            odd row even col = h
...
// saveGameState(game);
//module.exports = Game;
//# sourceMappingURL=quoridor.js.map